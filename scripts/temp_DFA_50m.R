#==================================================================================
#DFA on temperature metrics for 50m layer
...
wd <- getwd()
joined_50_dat <- read.csv("/Users/krista/Dropbox/Work folder/Pacific cod/Analysis/Temp4Krista/joined_data_for_DFA.csv",
                            row.names = 1)
joined_50_dat$type[which(joined_50_dat$type=="temp")] <- "CFSR_temp"
joined_50_dat <- joined_50_dat[!duplicated(joined_50_dat),]
wide_50_dat <- pivot_wider(joined_50_dat, names_from = type, values_from = temp)

scaled_dat <- wide_50_dat %>%     #UPDATE HERE when all indices are in hand IMPORTANT
  mutate(mean_jun_temp_hycom_scaled=scale(mean_jun_temp_hycom),
