#==================================================================================
#DFA on temperature metrics for 100m layer
...
wd <- getwd()
joined_100_dat <- read.csv("/Users/krista/Dropbox/Work folder/Pacific cod/Analysis/Temp4Krista/joined_100m_data_for_DFA.csv",
                           row.names = 1)
joined_100_dat$type[which(joined_100_dat$type=="mean_jun_temp_100m_40_60")] <- "CFSR_mean_jun_temp_100m_40_60"
joined_100_dat <- joined_100_dat[!duplicated(joined_100_dat),]
wide_100_dat <- pivot_wider(joined_100_dat, names_from = type, values_from = temp)

scaled_dat <- wide_100_dat %>%     #UPDATE HERE when all indices are in hand IMPORTANT
  mutate(mean_jun_temp_hycom_100_scaled=scale(mean_jun_temp_hycom_100),
         CFSR_mean_jun_temp_100m_40_60_scaled=scale(CFSR_mean_jun_temp_100m_40_60),
         AFSC_BTS_scaled=scale(AFSC_BTS),
         AFSC_LLS_scaled=scale(AFSC_LLS),
         IPHC_FISS_scaled=scale(IPHC_FISS),
         GAK1_mooring_90m_to_110m_scaled=scale(GAK1_mooring_90m_to_110m))

#plot=======

scaled_long <- scaled_dat[,c(1,8:13)] %>% pivot_longer(-year, names_to = "metric", values_to = "temp_value")
...
scaled_dat1 <- scaled_dat[,c(1,8:13)] #only scaled cols, update when all cols are in hand
